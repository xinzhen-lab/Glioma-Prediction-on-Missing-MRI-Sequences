...
if __name__ == '__main__':
    ...
    feature_name = pd.read_csv('D:/Glioma/2-Features/test_files/Grading/Test/T2-FLAIR.csv').columns[2: 111]
    ...
    for ite in [1, 2, 3, 4, 5]:
        ...
        "The matrix to save shap values"
        Shap_values_test = np.zeros((num_epochs, N_test, M * d))
        ...
        for epoch in range(num_epochs):
            ...
            "Calculate SHAP values for each iteration with dimensions (N_test, d)"
            ...
            Shap_values_test[epoch] = np.concatenate(shap_values[0], axis=-1)
        ...
        print(
            f'Best Train Result - AUC: {auc_list[epoch]}, ACC: {acc_list[epoch]}, SEN/Recall:{sen_list[epoch]}, SPE:{spe_list[epoch]},'
            f'NPV:{npv_list[epoch]}, Precision/PPV:{precision_list[epoch]}, F1-score:{f1score_list[epoch]}, total_Loss: {total_list[epoch]}')
        print(
            f'Best Test Result - AUC: {auc_list_[epoch]}, ACC: {acc_list_[epoch]}, SEN/Recall:{sen_list_[epoch]}, SPE:{spe_list_[epoch]},'
        ...
        # best_shap_values = Shap_values_test[epoch]
        # np.save(sub_Path2 + '/shap_values.npy', best_shap_values)
